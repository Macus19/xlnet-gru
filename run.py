...
from XLnet_GRU import XLNetForSequenceClassification_GRU
from transformers import AdamW, get_linear_schedule_with_warmup, XLNetConfig, XLNetTokenizer
from metrics import acc_and_f1
from utils import output_modes as output_modes
from utils import processors as processors
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    out_imprisonment_label_ids = None
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            if args.model_type != "distilbert":
                inputs["token_type_ids"] = batch[2] if args.model_type in ['bert', 'xlnet', 'albert'] else None 
            outputs = model(**inputs)
            tmp_eval_loss, logits, imprisonment_logits = outputs[:3]

            eval_loss += tmp_eval_loss.mean().item()
        nb_eval_steps += 1
        if preds_1 is None:
            preds_1 = logits.detach().cpu().numpy()
            preds_2 = imprisonment_logits.detach().cpu().numpy()
            out_label_ids = inputs["labels"].detach().cpu().numpy()
            out_imprisonment_label_ids = inputs["imprisonment_labels"].detach().cpu().numpy()
        else:
            preds_1= np.append(preds_1, logits.detach().cpu().numpy(), axis=0)
            preds_2 = np.append(preds_2, imprisonment_logits.detach().cpu().numpy(), axis=0)
            out_label_ids = np.append(out_label_ids, inputs['labels'].detach().cpu().numpy(), axis=0)
            out_imprisonment_label_ids = np.append(out_imprisonment_label_ids, inputs['imprisonment_labels'].detach().cpu().numpy(), axis=0)
    ...
    result_1 = acc_and_f1(preds_1, out_label_ids)
    result_2 = acc_and_f1(preds_2, out_imprisonment_label_ids)
    result_1.update(result_1)
    result_2.update(result_2)

    output_eval_file = os.path.join(args.output_dir, prefix, "eval_results.txt")
    with open(output_eval_file, "w") as writer:
        logger.info("***** Eval results {} *****".format(prefix))
        for key in sorted(result_1.keys()):
            logger.info("  %s = %s", key, str(result_1[key]))
            writer.write("%s = %s\n" % (key, str(result_1[key])))
        for key in sorted(result_2.keys()):
            logger.info("  %s = %s", key, str(result_2[key]))
            writer.write("%s = %s\n" % (key, str(result_2[key])))
...
def predict(args, model, tokenizer, prefix=""):
    ...
    pd.DataFrame(preds_1, out_label_ids).to_csv(os.path.join(args.output_dir, "predicted.csv"), index=False)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, predict=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        imprisonment_label_list = processor.get_imprisonment_labels()
        ...
        features = convert_examples_to_features(examples,
                                                ...
                                                imprisonment_label_list=imprisonment_label_list,
    ...
    if output_mode == "classification":
        ...
        all_imprisonment_labels = torch.tensor([f.imprisonment_label for f in features], dtype=torch.long)
    elif output_mode == "regression":
        all_labels = torch.tensor([f.label for f in features], dtype=torch.float)
 
    dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_labels, all_imprisonment_labels)
    return dataset
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result_1, result_2 = evaluate(args, model, tokenizer, prefix=prefix)
            result_1 = dict((k + '_{}'.format(global_step), v) for k, v in result_1.items())
            result_2 = dict((k + '_{}'.format(global_step), v) for k, v in result_2.items())
            results_1.update(result_1)
            results_2.update(result_2)
